...
export default function AnalyzeSkinPage() {
  ...
  const [urgentNotes, setUrgentNotes] = useState<string[]>([])
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const endpoint = process.env.NEXT_PUBLIC_API_URL
        ? `${process.env.NEXT_PUBLIC_API_URL}/analyze-skin`
        : "/api/analyze-skin";
      ...
      if (data.conditions && data.recommendations) {
        ...
        setUrgentNotes(data.urgent_notes || [])
  ...
  const handleSaveReport = async () => {
    if (!analysisComplete || conditions.length === 0 || recommendations.length === 0) {
      toast({
        title: "No Analysis to Save",
        description: "Please complete an analysis first before saving the report.",
        variant: "destructive"
      })
      return
    }
    ...
    try {
      const response = await fetch('/api/skin-reports', {
        ...
        body: JSON.stringify({
          ...
          recommendations,
          urgentNotes,
          imageData: uploadedImage, // Optional: save the analyzed image
          reportName: `Skin Analysis - ${new Date().toLocaleDateString()}`
        })
      })
      ...
      if (response.ok) {
        toast({
          ...
          description: "Your skin analysis report has been saved to your dashboard."
        })
      } else {
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <CliniQLogo size="sm" />
              <h1 className="text-xl font-bold text-gray-800">Clini-Q - Skin Analyzer</h1>
            </div>
          </div>
        </div>
      </header>
