...
export default function FullDashboardPage() {
  ...
  const fetchSkinReports = async () => {
    try {
      setLoadingReports(true)
      const response = await fetch("/api/skin-reports")
      if (response.ok) {
        const data = await response.json()
        setSkinReports(data.reports || [])
      }
    } catch (error) {
  ...
  const deleteReport = async (reportId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        fetchSkinReports() // Refresh the reports list
      } else {
        const errorData = await response.json()
        toast({
          title: "Error",
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <CliniQLogo size="sm" />
                <h1 className="text-xl font-bold text-gray-800">Clini-Q Dashboard</h1>
              </div>
            </div>
      ...
      <div className="text-center py-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          ...
          <TabsContent value="reports" className="space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-semibold text-gray-800">Skin Analysis Reports</h3>
              <Button 
                className="bg-pink-600 hover:bg-pink-700"
                onClick={fetchSkinReports}
              ...
              {loadingReports ? (
                ...
                <Card className="border-pink-100">
                  <CardContent className="p-8 text-center">
                    ...
                    <p className="text-gray-600 mb-2">No skin analysis reports yet</p>
                    <p className="text-sm text-gray-500 mb-4">Complete a skin analysis and save the report to see it here</p>
                    <Button
                ...
                skinReports.map((report) => (
                  <Card key={report.id} className="border-pink-100">
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        {/* Detected Conditions */}
                        <div>
                          <h4 className="font-medium text-gray-800 mb-2">Detected Conditions</h4>
                          <div className="space-y-2">
                            {report.conditions?.map((condition: any, index: number) => (
                              <div
                                key={index}
                        ...
                        {/* Urgent Notes */}
                        {report.urgentNotes && report.urgentNotes.length > 0 && (
                          <div>
                            ...
                            <div className="space-y-1">
                              {report.urgentNotes.map((note: string, index: number) => (
